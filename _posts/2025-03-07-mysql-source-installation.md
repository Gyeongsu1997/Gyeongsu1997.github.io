...
/usr/local/mysql/bin 디렉토리에는 mysql 서버와 클라이언트 등의 실행 파일이 위치합니다. 다음으로 mysql 서버를 초기화하기 위해 **sudo ./mysqld --initialize --user=mysql**를 실행하면 root@localhost 계정에 대한 임시 비밀번호가 생성됩니다. 최초로 로그인하기 위해 이 비밀번호를 기억해두어야 합니다.
