...
스타일드 컴포넌트는 대표적인 CSS-in-JS 라이브러리입니다. CSS-in-JS란 자바스크립트 파일 안에서 CSS 스타일을 작성할 수 있게 해주는 기술입니다. 스타일드 컴포넌트를 사용하면 손쉽게 재사용 가능한 UI 컴포넌트를 만들고 동적으로 스타일을 적용할 수 있다는 장점이 있습니다. 오늘의 포스팅에서는 스타일드 컴포넌트에서 사용되는 tagged templates이라는 문법에 대해 알아본 뒤 직접 스타일드 컴포넌트를 구현해보겠습니다.
