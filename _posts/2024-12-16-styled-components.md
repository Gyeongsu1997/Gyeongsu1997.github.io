...
스타일드 컴포넌트는 대표적인 CSS-in-JS 라이브러리입니다. 스타일드 컴포넌트를 사용하면 CSS 셀렉터가 필요없이 재사용 가능한 컴포넌트를 스타일링 할 수 있습니다. 오늘의 포스팅에서는 스타일드 컴포넌트에서 사용되는 tagged templates이라는 문법에 대해 알아본 뒤 직접 스타일드 컴포넌트를 구현해보겠습니다. 
