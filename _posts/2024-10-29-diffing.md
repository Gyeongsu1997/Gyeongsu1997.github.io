...
DOM에서 변경된 부분만 찾기 위해 루트 노드부터 비교를 시작합니다.
...
2. 자식 노드가 새롭게 추가된 경우
3. 자식 노드가 텍스트 노드인 경우
4. 기존 자식 노드와 새 자식 노드의 태그 이름이 다른 경우
5. 기존 노드와 새 노드의 태그 이름이 같은 경우

위 내용을 루트 노드부터 시작하여 자식 노드까지 재귀적으로 반복하면 변경이 있는 부분만 루트 노드에 반영됩니다. 위 내용을 코드로 표현한 것은 아래와 같습니다.
